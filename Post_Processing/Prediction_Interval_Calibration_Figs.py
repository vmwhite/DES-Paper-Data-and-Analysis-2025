...
import sys
sys.path.append('C:/Users/vmwhite/Documents/GitHub/DaneCountyOpioidSimulation') 
...
csv_files = ['\Base_Yearly_Arrivals.csv','\Base_Yearly_ODeaths.csv','\Base_Yearly_Arrests.csv', '\Base_Yearly_Indv_Treats.csv', '\Base_Yearly_Hosp.csv', '\Base_Yearly_Prevalence.csv']
...
for idx, item in enumerate(comp_inputs):
    ...
    if idx == 5:
        pi_graph(dict_mu, dict_max, dict_min, dict_sd(df,num_years),inputs.df_prev["Year"], inputs.df_prev["Dane County use estimate (number of people) LOWER CI"].astype(float),inputs.df_prev["Dane County use estimate (number of people)"].astype(float),"Prevalence", start_year, num_years, warmup, n_runs)
    else:
        pi_graph_point(dict_mu, dict_max, dict_min, dict_sd(df,num_years),item,ecol_lab[idx], x_lab[idx], start_year, num_years, warmup, n)
